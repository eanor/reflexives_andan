...
df <- read.csv("df1_CHECKED_23_06_25_1.csv", header = TRUE, stringsAsFactor = TRUE) # сюда вставляешь название файла
...
process_reading_data <- function(data_filtered, reading_type) {
  ...
  plot1 <- ggplot(answer_counts, aes(x = "", y = n, fill = ANSWER)) +
    ...
    labs(title = paste("Answer distribution -", reading_type, " (1 experiment)"),
         fill = "Answer") +
    theme_linguistic() +
  ...
  plot2 <- ggplot(conj_data, aes(x = CONJ, y = percentage, fill = ANSWER)) +
    ...
    labs(title = "By conjunction type (1 experiment)",
  ...
  plot3 <- ggplot(ic_data, aes(x = IC, y = percentage, fill = ANSWER)) +
    ...
    scale_fill_manual(values = professional_palette[1:2]) +
    scale_y_continuous(labels = scales::percent_format()) +
    labs(title = "By IC type (1 experiment)",
  ...
  plot4 <- ggplot(context_data, aes(x = context, y = percentage, fill = ANSWER)) +
    ...
    scale_fill_manual(values = professional_palette[1:2]) +
    scale_y_continuous(labels = scales::percent_format()) +
    labs(title = "By context type (1 experiment)",
  ...
  plot5 <- ggplot(contribution_data, 
    ...
    labs(title = paste("Parameter contribution to", reading_type, "reading acceptance (1 experiment)"),
  ...
  plot6 <- ggplot(contribution_data, 
    ...
    scale_fill_gradient(low = professional_palette[3], 
                        high = professional_palette[4],
                        name = "Rejection rate %") +
    labs(title = paste("Parameter contribution to", reading_type, "reading rejection (1 experiment)"),
         x = "Conjunction & IC type",
         y = "Context") +
    theme_linguistic() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
create_mosaic_gg(df_sloppy, ~ ANSWER + IC + context, "IC × context interaction (sloppy reading), 1 experiment")
create_mosaic_gg(df_sloppy, ~ ANSWER + context + CONJ, "context × CONJ interaction (sloppy reading), 1 experiment")
create_mosaic_gg(df_sloppy, ~ ANSWER + IC + CONJ, "IC × CONJ interaction (sloppy reading), 1 experiment")

create_mosaic_gg(df_strict, ~ ANSWER + IC + context, "IC × context interaction (strict reading), 1 experiment")
create_mosaic_gg(df_strict, ~ ANSWER + context + CONJ, "context × CONJ interaction (strict reading), 1 experiment")
create_mosaic_gg(df_strict, ~ ANSWER + IC + CONJ, "IC × CONJ interaction (strict reading), 1 experiment")
...
ggplot(df_sloppy) +
  ...
  labs(
    title = "Parameter Interaction: context × CONJ × IC (1 experiment)",
    x = "ANSWER:CONJ",
...
plot(allEffects(model_sloppy),
     main = "Effects on sloppy reading acceptance (1 experiment)",
...
ggplot(var_imp_sloppy, aes(x = reorder(Term, `Pr(>Chi)`), y = -log10(`Pr(>Chi)`))) +
  ...
  coord_flip() +
  labs(title = "Parameter importance for sloppy reading acceptance  (1 experiment)",
       x = "Parameter",
       y = "-log10(p-value)") +
  theme_linguistic()
...
ggplot(var_imp_strict, aes(x = reorder(Term, `Pr(>Chi)`), y = -log10(`Pr(>Chi)`))) +
  geom_bar(stat = "identity", fill = "steelblue") +
  coord_flip() +
  labs(title = "Parameter importance for strict reading acceptance (1 experiment)",
       x = "Parameter",
       y = "-log10(p-value)") +
  theme_linguistic()
